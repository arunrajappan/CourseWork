...
def readTrainTestFileList(pTrainFile=None,pTestFile=None):
    ...
    if(pTestFile is None):
        pTestFile= 'testFileListOld'#'testFileList'
    # print(pTrainFile)
...
'''
def processTrainingFiles(oper="extractTrainingVocabulary",fileType="train",isRealtime=False):
    global saveTrainFeaturesDes, visuals
    ...
    if(isRealtime):
        processVideoData(oper,fileType,'0',"8",isRealtime)
    else:
        for label, fileList in trainTestFiles.iteritems():
            ...
            for file in fileList:
                frameCounter = 0
                print(label,file)
                processVideoData(oper,fileType,file,label,isRealtime)


    cv2.destroyAllWindows()

'''
Function to Test on real time data
'''
def processVideoData(oper="extractTrainingVocabulary",fileType="train",file = "0",label="8", isRealtime=False):
    global saveTrainFeaturesDes, visuals
    print("Processing Video..")
    if(isRealtime):
        processRealTimeVideo(oper,fileType,file,label,isRealtime)
    else:
        processArchievedVideo(oper,fileType,file,label,isRealtime)

'''
Function to Test on real time data
'''
def processRealTimeVideo(oper="extractTrainingVocabulary",fileType="train",file = "0",label="8", isRealtime=True):
    ...
    while(True):
        ...
        if(subFrameCount == 0):
            ...
            visCopy = vis.copy()
            draw_str(visCopy, (20, 20), 'Label -'+myLabel)
        ...
        if(subFrameCount > skipFrames and subFrameCount7 > skipFrames7 and subFrameCount14 > skipFrames14):
            ...
            if(hasAtleastOneKP):
                ...
                print("testData Shape: ", np.float32(testData).shape)
                ...
                print("testData reShaped: ",testData.shape)
                print("testLabels reShaped: ", len(testLabels))
                # print("testData : ", testData)
                # print("testLabels : ", testLabels)
            ...
            draw_str(visCopy14, (20, 20), 'Label - '+myLabel)
            cv2.imshow('Output', visCopy)
...
Function to Test on real time data
'''
def processArchievedVideo(oper="extractTrainingVocabulary",fileType="train",file = "0",label="8", isRealtime=False):
...
if __name__ == '__main__':
    ...
    if not(onlyTest):
        ...
        trainData = np.float32(trainData).reshape(-1,3*dictSize)
    ...
    processTrainingFiles(oper="extractBOWDescriptor", fileType="test", isRealtime=True)
    ...
    print("Result Mask: ", mask.reshape(1,-1));
